...
os.system("cp silicon.win wannier90.win")
...
num_kpts_loc = 64
mp_grid_loc = [4, 4, 4]
real_lattice_loc = np.asarray(
    [[-2.6988, 0.0000, -2.6988], [0.0000, 2.6988, 2.6988], [2.6988, 2.6988, 0.0000]],
    dtype=float,
)  # in C order
...
num_atoms_loc = 2
atom_symbols_loc = ["Si", "Si"]
atom_atomic_loc = ["14", "14"]
atoms_frac_loc = np.asarray([[-0.25, 0.75, -0.25], [0.00, 0.00, 0.00]])  # in C order
atoms_cart_loc = atoms_frac_loc.dot(real_lattice_loc)
...
for i in range(num_kpts_loc):
    for j in range(num_wann_loc):
        for k in range(num_bands_loc):
            x = float(
                lines[i * num_wann_loc * num_bands_loc + j * num_bands_loc + k][3]
            )
            y = float(
                lines[i * num_wann_loc * num_bands_loc + j * num_bands_loc + k][4]
            )
...
M_matrix_loc = np.empty(
    [num_kpts_loc, nntot_loc, num_bands_loc, num_bands_loc], dtype=complex
)
file = open(name + ".mmn")
file.readline()
file.readline()
...
for nkp in range(num_mmn):
    ...
    M_kpt2[nkp, :] = line
    for k in range(num_bands_loc):
        for l in range(num_bands_loc):  # noqa: E741
            lines.append(file.readline().split())
...
U_matrix, U_matrix_opt, lwindow, wann_centres, wann_spreads, spread = libwannier90.run(
    seed__name,
    mp_grid_loc,
    num_kpts_loc,
    real_lattice_loc,
    recip_lattice_loc,
    kpt_latt_loc.flatten(order="F"),
    num_bands_tot,
    num_bands_loc,
    num_wann_loc,
    nntot_loc,
